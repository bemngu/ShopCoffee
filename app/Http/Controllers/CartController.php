...
class CartController extends Controller
{
    ...
    public function add_cart_ajax(Request $request){
        // Session::forget('cart');
        $data = $request->all();
        $session_id = substr(md5(microtime()),rand(0,26),5); //mỗi sản phẩm thêm vào nó sẽ tự tạo ra 1 ssion_id lấy ra 5 ký tư
        $cart = Session::get('cart');
        if($cart==true){
            $is_avaiable = 0; //xem cart tồn tại hay ch
            foreach($cart as $key => $val){
                if($val['product_id']==$data['cart_product_id']){
                    $is_avaiable++; //xem cái cart có chưa...nếu có tăng lên 1
                }
            }
            if($is_avaiable == 0){ //k có cái cart nào trùng thì tạo ra 1 cart mới 
                $cart[] = array(
    ...
    public function delete_product($session_id){
        ...
        // print_r($cart);
        // echo '</pre>';
        if($cart==true){ //nếu tồn tại cái ssion cart giỏ 
            foreach($cart as $key => $val){
                if($val['session_id']==$session_id){
                    unset($cart[$key]); //keys số thứ tự của mảng
                }
            }
            Session::put('cart',$cart); //put là đăt lại cái cart
    ...
    public function update_cart(Request $request){
        ...
        if($cart==true){
            ...
            foreach($data['cart_qty'] as $key => $qty){ //name bên cart_ajax
                $i = 0;
    ...
    public function delete_all_product(){
        $cart = Session::get('cart'); //tồn tại cart
        if($cart==true){
            // Session::destroy();
            Session::forget('cart');
